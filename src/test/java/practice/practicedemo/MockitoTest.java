...
//        verify(list).add(argThat(e->e.length()>5));
    ...
    public void testMockAndInjectMock() {
        // 设置存根
        when(bar.add(1,2)).thenReturn(4);

        int result = foo.sum(1,2);
        int result2 = foo.sum(1,2);
    ...
    public void testAnswer() {
        ...
        when(bar.add(1,2)).thenAnswer(answer);
    ...
    public void testAssertV3() {
        Bar bar = mock(Bar.class);
        when(bar.add(1,2)).thenReturn(7);
        assert  bar.add(1,2) == 7 : "true,不会打印出断言信息";
        assert  bar.add(1,2) == 3 : "false,不会打印出断言信息";
    }
